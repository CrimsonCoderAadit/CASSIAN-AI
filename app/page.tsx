        ...
        >
          {loading ? (
            ...
            <motion.button
              ...
              disabled={signingIn}
              className="glow-neon-sm inline-flex items-center gap-3 rounded-full border border-border bg-surface px-8 py-3 text-sm font-medium text-foreground transition-all duration-300 hover:border-neon/30 hover:bg-surface-hover disabled:cursor-not-allowed disabled:opacity-50"
              whileTap={{ scale: 0.95 }}
              whileHover={{ scale: signingIn ? 1 : 1.02 }}
            >
              {signingIn ? (
                <>
                  <div className="h-[18px] w-[18px] animate-spin rounded-full border-2 border-neon/20 border-t-neon" />
                  Signing in...
                </>
              ) : (
                <>
